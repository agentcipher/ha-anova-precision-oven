...
def mock_settings():
    """Mock settings."""
    with patch('custom_components.anova_oven.anova_sdk.oven.settings') as mock:
...
def mock_client():
    ...
    with patch('custom_components.anova_oven.anova_sdk.oven.WebSocketClient') as mock:
...
def setup_oven(mock_settings):
    ...
    with patch('custom_components.anova_oven.anova_sdk.oven.WebSocketClient'):
        with patch('custom_components.anova_oven.anova_sdk.oven.setup_logging'):
            oven = AnovaOven()
...
class TestAnovaOvenStartCookAdvanced:
    ...
    async def test_start_cook_stage_construction_validation_error(self, setup_oven):
        ...
        with patch('custom_components.anova_oven.anova_sdk.oven.CookStage') as mock_cook_stage:
    ...
    async def test_start_cook_logging_both_units(self, setup_oven):
        ...
        with patch.object(oven.command_builder, 'build_start_command') as mock_build:
            ...
            with patch('custom_components.anova_oven.anova_sdk.oven.settings') as mock_settings:
